...
前節でまとめたように、Ti にとって不可視な Tj は以下の2つのケースがあります：
...
abort は手動と自動の2種類があります。手動は abort() メソッドの呼び出しで、自動はデッドロック検出やバージョンジャンプ検出時に自動的にロールバックされます：
...
`delete()` はやや複雑ですが、主に以下の3つの前処理を行います：可視性の判定、リソースロックの取得、バージョンジャンプの判定。削除操作自体は XMAX の設定のみです。
